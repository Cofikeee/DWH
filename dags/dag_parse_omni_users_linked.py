...
async def worker(queue, session, pool):
    ...
    while True:
        ...
        await OmniUserProcessor(user_id, session, pool).process_user()
        queue.task_done()


async def fetch_and_process_users_linked():
    """
    ...
    from_time = qs.select_max_ts('dim_omni_user', 'created_date')
