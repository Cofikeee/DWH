...
async def collect_agg_n_sms_d(conn, from_created_date, to_created_date):
    query = f"""
        SELECT person.user_id AS dim_user_id,
               ...
               DATE_TRUNC('day', sms_notification.created_date)::DATE AS dim_start_of_day,
...
async def collect_agg_s_session_d(conn, from_created_date, to_created_date):
         ...
         WITH
             autorization AS (
                 SELECT user_id                         
                      , id
                      , created_date
                      , DATE_TRUNC('day', created_date)::DATE AS start_of_day
                     , CASE
    ...
    try:
        ...
        print(f"Ошибка PostgreSQL при выполнении запроса: {e.__class__.__name__} - {e}")
        raise
...
async def collect_agg_c_signing_notification_sms_d(conn, from_created_date, to_created_date):
    query = f"""
            SELECT person.user_id AS dim_user_id,
                   last_name || ' ' || first_name || ' ' || COALESCE(patronymic, ' ') AS dim_person_fio,
                   DATE_TRUNC('day', ca_document_signing_request.created_date)::DATE AS dim_start_of_day,
...
async def collect_dim_user_login_scd2(conn, from_modified_date, to_modified_date):
    query = f"""
        SELECT user_login.user_id                               AS user_id
             ...
             , user_login.confirmed_date::TIMESTAMP(0)          AS confirmed_date
             , notification_channel.modified_date::TIMESTAMP(0) AS modified_date
...
async def collect_agg_e_doc_type_m(conn, from_modified_date, to_modified_date):
    query = f"""
        SELECT date_trunc('month', document.created_date)::DATE as start_of_month
             , COUNT (DISTINCT document_type_id) as agg_cnt_doc_type
        FROM ekd_ekd.document
        WHERE base_document_id IS NOT NULL AND docflow_finished_date IS NOT NULL
          AND sent_date >= '{from_modified_date}' AND sent_date <= '{to_modified_date}'
        GROUP BY start_of_month;
    """
    return await conn.fetch(query)


async def collect_agg_e_app_type_m(conn, from_modified_date, to_modified_date):
    query = f"""
        SELECT date_trunc('month', application.created_date)::DATE as start_of_month
             , COUNT(DISTINCT application_type_id) as agg_cnt_app_type
        FROM ekd_ekd.application
             JOIN ekd_ekd.application_group ON application.application_group_id = application_group.id
        WHERE docflow_finished_date IS NOT NULL
          AND application.created_date >= '{from_modified_date}' AND application.created_date <= '{to_modified_date}'
        GROUP BY start_of_month;
    """
    return await conn.fetch(query)
